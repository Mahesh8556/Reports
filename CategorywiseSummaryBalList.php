...
$filename = __DIR__.'/CategoryWiseSummaryBalList.jrxml';
...
$query = '  SELECT "ACNOTYPE", "AMOUNT_FROM",  "AMOUNT_TO" 
, SUM(CASE CAST("LEDGER_BALANCE" AS FLOAT) WHEN '.$zero.' THEN '.$zero.' ELSE '.$one.' END) "LEDGER_ACCOUNTS", 
SUM(CAST("LEDGER_BALANCE" AS FLOAT)) "LEDGER_BALANCE"  
FROM SIZEWISEBALANCE, 
( SELECT VWALLMASTER."ac_acnotype" AS "AC_ACNOTYPE" , VWALLMASTER."ac_type" "AC_TYPE", VWALLMASTER."ac_no" AS "AC_NO" , 
...
AND VWALLMASTER."ac_acnotype" = '.$acno.' AND VWALLMASTER."ac_type" ='.$actype.' 
 ...
 AND (ABS(CAST(TMP."LEDGER_BALANCE" AS FLOAT)) <=  SIZEWISEBALANCE."AMOUNT_TO" )
         GROUP BY SIZEWISEBALANCE."ACNOTYPE", SIZEWISEBALANCE."AMOUNT_FROM",  SIZEWISEBALANCE."AMOUNT_TO" 
';
