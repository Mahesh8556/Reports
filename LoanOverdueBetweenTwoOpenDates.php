...
$filename = __DIR__ . '/LoanOverdueBetweenTwoOpenDates.jrxml';
$space = "' '";
$data = [];
	...
	(SELECT LNMASTER.ID "LNID",
			...
			OIRINTBALANCE(SCHEMAST."S_APPL",
                ...
                (case when CAST (LNMASTER."AC_INSTALLMENT" as FLOAT) <> 0  then CEIL(DUEBALANCE(CAST(SCHEMAST."S_APPL" AS CHARACTER VARYING),
                                    ...
                                    (case when CAST (LNMASTER."AC_INSTALLMENT" as FLOAT) <> 0  then CEIL((CAST(LNMASTER."AC_SANCTION_AMOUNT" AS FLOAT) - DUEBALANCE(CAST(SCHEMAST."S_APPL" AS CHARACTER VARYING),
					...
					(SELECT *
						FROM
							(SELECT "TRAN_ACNOTYPE",
								...
								FROM LOANTRAN
								WHERE CAST("TRAN_DATE" AS date) <= TO_DATE('.$date1.',
		...
		ORDER BY SCHEMAST."S_APPL",
            ...
            AND CAST(S."AC_OPDATE" AS DATE) <= TO_DATE('.$date1.','.$dateFormat.')
            AND (S."AC_OPDATE" IS NULL OR CAST(S."AC_OPDATE" AS DATE) >= TO_DATE('.$date.','.$dateFormat.'))
            AND (S."AC_CLOSEDT" IS NULL OR CAST(S."AC_CLOSEDT" AS DATE) > TO_DATE('.$date1.','.$dateFormat.'))';
}
else
{
    $query='SELECT *
...
while ($row = pg_fetch_assoc($sql)) {
    ...
    $temp = [
        ...
        "totalbal" => sprintf("%.2f", ($GrandTotal_5) + 0.0),
    ...
    //      'AccountHolderName'=>$row['AC_NAME'],
    //      //'Guarantor_Name'=>$row[''],
    //      //No database coulmn available for guarantor name
    //      'city'=>$row['CITYNAME'],
    //      'OpeningDate'=>$row['AC_OPDATE'],
    //      'ExpiryDate'=>$row['AC_EXPIPRE_DATE'],
    //      'SanctionedLoanAmount'=>$row['AC_SANCTION_AMOUNT'],
    //      'InstallmentAmount'=>$row['AC_INSTALLMENT'],
    ...
    //      'DueBalance'=>$row['DUEBALANCE'],
    //      'DueInstallment'=>$row['DUEINSTALLMENT'],
    //      'SL' =>sprintf("%.2f", (abs($row['AC_SANCTION_AMOUNT']))).' '.$netType, 
    //     //  'IA' =>sprintf("%.2f", (abs($row['Scheme']))).' '.$netType,
    //     //  'RI' =>sprintf("%.2f", (abs($row['Scheme']))).' '.$netType,
    //     //  'OB' =>sprintf("%.2f", (abs($row['Scheme']))).' '.$netType,
    //     //  'DB' =>sprintf("%.2f", (abs($row['Scheme']))).' '.$netType,
    //     //  'TOA' =>sprintf("%.2f", (abs($row['Scheme']))).' '.$netType, 
    //  ];
    $data[$i] = $temp;
    $i++;
}
